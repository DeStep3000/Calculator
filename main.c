...
complex double get_variable_value(char *name) {
...
void set_variable_value(char *name, complex double value) {
    for (int i = 0; i < num_vars; i++) {
        if (strcmp(vars[i].name, name) == 0) {
            vars[i].value = value;
            return;
...
complex double apply_operator(char op, complex double x, complex double y) {
    switch (op) {
        case '+':
            return x + y;
        case '-':
...
complex double evaluate_expression(char *expr) {
    int operator_top = -1;
    char operator_stack[MAX_EXPR_LEN];
    complex double value_stack[MAX_EXPR_LEN];
    int value_top = -1;
    int i = 0;
    int unary_minus = 0;
    while (expr[i] != '\0' && expr[i] != '\n') {
        if (isspace(expr[i])) {
            ...
            name[j] = '\0';
            if (strcmp(name, "PI") == 0) {
                value_stack[++value_top] = unary_minus ? -M_PI : M_PI;
            } else if (strcmp(name, "e") == 0) {
                value_stack[++value_top] = unary_minus ? -M_E : M_E;
            } else if (strcmp(name, "j") == 0) {
                value_stack[++value_top] = unary_minus ? -I : I;
            } else {
                complex double value = get_variable_value(name);
            ...
            if (op == '-' && (i == 0 || is_operator(expr[i - 1]) || expr[i - 1] == '(')) {
                ...
                while (operator_top >= 0 && is_operator(operator_stack[operator_top]) &&
                    ...
                    complex double y = value_stack[value_top--];
                    complex double x = value_stack[value_top--];
...
int main() {
    ...
    complex double result = evaluate_expression(expression);
